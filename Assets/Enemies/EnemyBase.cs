...
namespace Enemies
{
    ...
    public class EnemyBase : MonoBehaviour
    {
        ...
        [SerializeField] protected float stopRadius = 1.5f;
        ...
        // ── Contact damage — handles both solid and trigger ship colliders ────
        protected virtual void OnCollisionEnter2D(Collision2D col) => TryContactDamage(col.gameObject);
        protected virtual void OnCollisionStay2D(Collision2D col)  => TryContactDamage(col.gameObject);
        protected virtual void OnTriggerEnter2D(Collider2D col)    => TryContactDamage(col.gameObject);
        protected virtual void OnTriggerStay2D(Collider2D col)     => TryContactDamage(col.gameObject);

        private void TryContactDamage(GameObject other)
        {
            if (_contactTimer > 0f) return;
            var sh = other.GetComponent<RocketShip.ShipHealth>();
            if (sh == null) return;
            ...
            var sc = other.GetComponent<RocketShip.ShipController>();
            if (sc != null)
            {
